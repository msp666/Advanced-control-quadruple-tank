...
ss_mp = uss(A,B,C,D);
Gp_mp = C/(s*eye(length(A)) - A)*B;
% from symbolic transfer function:
% Gp_mp = [gp1*kp1/(A1*s+ap1*sqrt(g/(2*h10))), -ap3*kp2*(gp2-1)*sqrt(g/(2*h30))/((A1*s+ap1*sqrt(g/(2*h10)))*(A3*s+ap3*sqrt(g/(2*h30))));...
%          -ap4*kp1*(gp1-1)*sqrt(g/(2*h40))/((A2*s+ap2*sqrt(g/(2*h20)))*(A4*s+ap4*sqrt(g/(2*h40)))), gp2*kp2/(A2*s+ap2*sqrt(g/(2*h30)))];
% G_mp = Gp_mp.NominalValue;
G_mp = zpk(Gp_mp.nominal); % normal TF (without uncertainty)
G_mp = tf(sys_mp);
% bodemag ( usample ( Gp_mp, 100 ) ) ;
%% Fit the weighting function
% construct closed loop with controller K_mp (Task 2 Q1)
K1 = 6; K2 = 8;
T1 = 25; T2 = 25; 
K_mp = [K1*(1+1/(T1*s)), 0; 0, K2*(1+1/(T2*s))];
...
for k = 1:1000
    ...
    D = zeros(size(C,1),size(B,2));
    G_temp = ss(A, B, C, D);
    % [Gp_mag(:, : ,:, k)] = squeeze(bode((G_temp - G_mp)*inv(G_mp), w));
    % for i = 1:length(w)
    %      Gf = freqresp((G_temp-G_mp)/G_mp, w(i));
    %      SV(k,i) = max(svd(Gf));
    % end   
    SV(k,:) = max( sigma((G_temp-G_mp)/G_mp, w), [], 1 );
...
Tpdyn_mp = feedback((eye(2) + WO*DeltaO)*G_mp*K_mp, eye(2));

figure;
bodemag(usample((Gp_mp-G_mp)/G_mp, 100)); hold on;
bodemag(WO*DeltaO,'r'); 
...
[margin_dyn, wcu_dyn] = robstab(Tpdyn_mp);
...
Tppara_mp = feedback(Gp_mp*K_mp, eye(2));
[margin_para, wcu_para] = robstab(Tppara_mp);
...
% plot lower bound of stability margin over frequency
w = logspace(-8,1,200); 
[~, ~, info_para] = robstab(Tppara_mp, w);
[~, ~, info_dyn] = robstab(Tpdyn_mp,w);
figure;
